...
class GridWorldEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        done = self._is_end_state(new_x, new_y) or self.step_cnt >= 200
        self.state = self._xy_to_state(new_x, new_y)
        # 提供格子世界所有的信息在info内
        info = {"x":new_x,"y":new_y, "grids":self.grids, "succ":int(self._is_end_state(new_x, new_y))}
...
def LargeGridWorld():
    ...
    env = GridWorldEnv(n_width=10,
                       ...
                       windy=False)
    env.start = (3,5)
    env.bad_ends = [(3,2), (3,6), (5,2), (6,2), (8,3),
                    (8,4), (6,4), (5,5), (6,5)]
    env.good_ends = [(5,4)]
    env.ends = env.bad_ends + env.good_ends
    env.types = [(4,2,1),(4,3,1),(4,4,1),(4,5,1),(4,6,1),(4,7,1),
                 (1,7,1),(2,7,1),(3,7,1),(4,7,1),(6,7,1),(7,7,1),
                 (8,7,1)]
    for x,y in env.bad_ends:
        env.rewards.append((x,y,-100))
    for x,y in env.good_ends:
        env.rewards.append((x,y,100))
    # env.rewards = [(3,2,-1),(3,6,-1),(5,2,-1),(6,2,-1),(8,3,-1),
    #                (8,4,-1),(5,4,10),(6,4,-1),(5,5,-1),(6,5,-1)]
    # env.rewards = [(5,4, 10)]
    env.refresh_setting()
    return env
